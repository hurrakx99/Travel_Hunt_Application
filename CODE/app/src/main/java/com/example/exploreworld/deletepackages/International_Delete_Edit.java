...
import com.example.exploreworld.adapters.DeleteInternationalAdapter;
...
public class International_Delete_Edit extends AppCompatActivity {
    RecyclerView recyclerView;
    List<PackageData> mydomesticviewlist;
   DeleteInternationalAdapter adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_international__delete__edit);
        recyclerView=findViewById(R.id.recycler_internationaldelete);
        mydomesticviewlist=new ArrayList<>();
        recyclerView.setLayoutManager(new GridLayoutManager(this,2));
        adapter=new DeleteInternationalAdapter(this,mydomesticviewlist);
        recyclerView.setAdapter(adapter);
        ConnectivityManager connectivityManager=(ConnectivityManager)getApplicationContext().getSystemService(Context.CONNECTIVITY_SERVICE);
        NetworkInfo infos=connectivityManager.getActiveNetworkInfo();
        if(null!=infos) {
        progressDialog=new ProgressDialog(International_Delete_Edit.this);
        progressDialog.setMessage("Loading....");
        progressDialog.show();
        databaseReference= FirebaseDatabase.getInstance().getReference("tourpackages").child("internationalpackage");
