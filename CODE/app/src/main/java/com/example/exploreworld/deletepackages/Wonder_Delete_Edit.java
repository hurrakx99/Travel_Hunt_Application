...
import com.example.exploreworld.adapters.DeleteWonderAdapter;
...
public class Wonder_Delete_Edit extends AppCompatActivity {
    RecyclerView recyclerView;
    List<PackageData> mydomesticviewlist;
    DeleteWonderAdapter adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_wonder__delete__edit);
        recyclerView=findViewById(R.id.recycler_wonderdelete);
        mydomesticviewlist=new ArrayList<>();
        recyclerView.setLayoutManager(new GridLayoutManager(this,2));
        adapter=new DeleteWonderAdapter(this,mydomesticviewlist);
        recyclerView.setAdapter(adapter);
        ConnectivityManager connectivityManager=(ConnectivityManager)getApplicationContext().getSystemService(Context.CONNECTIVITY_SERVICE);
        NetworkInfo infos=connectivityManager.getActiveNetworkInfo();
        if(null!=infos) {
        progressDialog=new ProgressDialog(Wonder_Delete_Edit.this);
        progressDialog.setMessage("Loading....");
        progressDialog.show();
        databaseReference= FirebaseDatabase.getInstance().getReference("tourpackages").child("wonderpackage");
